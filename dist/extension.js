...
function getModRoot() {
    const v = cfg('scsTools.modRoot', '${workspaceFolder}');
...
function getGameDataPath() {
    const explicit = cfg('scsTools.gameDataPath', '');
...
let cachedEts2Path;
function getEts2Path() {
    const configured = cfg('scsTools.ets2Path', '');
...
function parseSteamLibrariesFromVdf(vdfText) {
    const libs = new Set();
...
async function detectEts2InstallPath() {
    ...
    const patterns = [
        (d) => `${d}:\\\\SteamLibrary\\\\steamapps\\\\common\\\\Euro Truck Simulator 2`,
        (d) => `${d}:\\\\Steam\\\\steamapps\\\\common\\\\Euro Truck Simulator 2`,
    ];
...
function activate(context) {
    ...
    const openResourceCmd = vscode.commands.registerCommand('scsTools.openResource', async (args) => {
        try {
            const raw = String(args?.raw ?? '');
            const fromPath = args?.from ? String(args.from) : undefined;
            ...
            if (/^[a-zA-Z]:[\/]/.test(cleaned)) {
                ...
                if (gameDataRoot)
                    push(path.join(gameDataRoot, rel));
            }
            else {
                // Relative path
                ...
                if (gameDataRoot)
                    push(path.join(gameDataRoot, cleaned));
            }
    ...
    const colorProvider = vscode.languages.registerColorProvider('scs', {
        provideDocumentColors(document) {
            if (!cfg('scsTools.enableColorPicker', true))
                return [];
            const infos = [];
            ...
            for (let lineNo = 0; lineNo < document.lineCount; lineNo++) {
                ...
                if (!/colou?r/i.test(key))
                    continue; // keep it strict to avoid false positives
                // Tuples
                let m;
                while ((m = tupleRe.exec(line)) !== null) {
                    ...
                    if (!isFinite(r) || !isFinite(g) || !isFinite(b) || !isFinite(aRaw))
                        continue;
                ...
                while ((h = hexRe.exec(line)) !== null) {
                    ...
                    if (![rr, gg, bb, aa].every(v => isFinite(v)))
                        continue;
                    const start = h.index;
    ...
    const updateInlineColors = (editor) => {
        const e = editor || vscode.window.activeTextEditor;
        if (!e || e.document.languageId !== 'scs')
            return;
        if (!cfg('scsTools.enableInlineColorText', false)) {
        ...
        for (let lineNo = 0; lineNo < doc.lineCount; lineNo++) {
            ...
            while ((m = tupleRe.exec(line)) !== null) {
                ...
                const b = parseFloat(m[3]);
                const aRaw = m[4] !== undefined ? parseFloat(m[4]) : 1;
                if (![r, g, b, aRaw].every(v => isFinite(v)))
                    continue;
                const scale255 = Math.max(r, g, b, aRaw) > 1.0;
                const rr = clamp01(scale255 ? r / 255 : r);
                const gg = clamp01(scale255 ? g / 255 : g);
    ...
    context.subscriptions.push(vscode.window.onDidChangeActiveTextEditor(() => scheduleInlineUpdate()), vscode.workspace.onDidChangeTextDocument((e) => {
    ...
    function validate(document) {
        ...
        if (!cfg('scsTools.enableDiagnostics', true)) {
            diagnostics.set(document.uri, []);
            return;
        }
        const diags = [];
        ...
        for (let i = 0; i < document.lineCount; i++) {
            ...
            if (!RESERVED_SET.has(key)) {
                ...
                diags.push(new vscode.Diagnostic(range, `Unknown SCS key: "${key}" (you can disable this warning in settings: scsTools.enableDiagnostics).`, vscode.DiagnosticSeverity.Warning));
            }
        }
        diagnostics.set(document.uri, diags);
    }
    context.subscriptions.push(vscode.workspace.onDidOpenTextDocument(validate), vscode.workspace.onDidChangeTextDocument(e => validate(e.document)), vscode.workspace.onDidCloseTextDocument(doc => diagnostics.delete(doc.uri)), diagnostics);
    const getFormatOpts = () => ({
        enabled: cfg("scsTools.enableFormatter", true),
        onSave: cfg("scsTools.format.onSave", false),
        alignColons: cfg("scsTools.format.alignColons", true),
        maxEmptyLines: Math.max(0, Math.min(10, cfg("scsTools.format.maxEmptyLines", 1))),
        trimTrailingWhitespace: cfg("scsTools.format.trimTrailingWhitespace", true),
        spaceAfterCommaInTuples: cfg("scsTools.format.spaceAfterCommaInTuples", true),
        indent: cfg("scsTools.format.indent", "\t"),
    });
    function formatScsText(input, opts) {
        ...
        const flushGroup = () => {
            ...
            if (opts.alignColons) {
                ...
                for (const g of group)
                    out.push(`${g.indentStr}${g.key}: ${g.value}`.trimEnd());
            }
            group = [];
            groupLevel = null;
        };
        const isComment = (t) => t.startsWith("#") || t.startsWith("//");
        const isDirective = (t) => t.startsWith("@");
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (trimmed.length === 0) {
                flushGroup();
                emptyRun++;
                if (emptyRun <= opts.maxEmptyLines)
                    out.push("");
                continue;
            }
            emptyRun = 0;
    ...
    function makeFullReplaceEdits(document, newText) {
    ...
    context.subscriptions.push(vscode.workspace.onWillSaveTextDocument((e) => {
        const opts = getFormatOpts();
        if (!opts.enabled || !opts.onSave)
            return;
        if (e.document.languageId !== "scs")
            return;
        e.waitUntil(Promise.resolve().then(() => {
            const newText = formatScsText(e.document.getText(), opts);
            if (newText === e.document.getText())
                return [];
